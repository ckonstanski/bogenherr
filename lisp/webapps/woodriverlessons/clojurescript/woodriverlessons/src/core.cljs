...
(hiccups/defhtml template-about-us [jsonobj]
  ...
  [:h3 "The Philosophy"]
  [:p "Like all art, playing the violin or viola makes two demands on the artist. One must have artistic ideas worth expressing and also the technical craft that provides the means to express these ideas. The finest artistic thoughts ever conceived will never see the light of day if the performer lacks the technique to express them. Likewise the most tremendous technique in the world will not inspire the audience if it is not informed by artistic intent."]
