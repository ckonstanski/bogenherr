...
(hiccups/defhtml template-about-us [jsonobj]
  ...
  [:h3 "The Philosophy"]
  [:p "Playing the violin and viola is like any other art form in that it is composed of two parts: the artistic ideas which want to be expressed and the craft which provides the artist with the means to express them. We generally refer to the craft of instrumental performance as \"technique\". The finest artistic ideas ever conceived will never see the light of day if the performer lacks the technique to express them. Likewise the most tremendous technique in the world will not inspire the audience if it is not informed by artistic intent."]
